...
BEGIN
 ...
 loop
      update employees
      set salary=salary-1
      where current of  c_emp_dept30;    -- con esto se asegura que el update se estara 
	  -- dando siempre y cuando sea la misma fila. Usa a nivel del ROWID de la tabla = rowid del cursor
	  -- SOLO SE puede usar el CURRENT OF si es que se usar en la arte superior el DOR UPDATE
  end loop; 
  commit;
END;
---
