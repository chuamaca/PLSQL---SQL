...
v_no_rec number;
begin
execute immediate 'delete from '||p_table_name; -- se ejecutara en tiempo de ejecucion = run time
...
execute delete_any_table('dfd'); -- en este caso dara error en la ejecucion, no en la compilacion.
-----------------------------------------------------------
...
-- da error porque no se puede ejecutar un DDL dentro de un bloque plsql

-- pero si se puede ejecutar como execute immediate:
begin
